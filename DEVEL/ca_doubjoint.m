...
[~, ~, linjoints, ~] = WBPREPROC(pcs, bcs, linjoints, excs, wcomps);
[~, ~, fusjoints, ~] = WBPREPROC(pcs, bcs, fusjoints, excs, wcomps);
[pcs, bcs, nljoints, excs] = WBPREPROC(pcs, bcs, nljoints, excs, wcomps);
...
[Amat, ~, ~, Fv] = WVAMAT([Wst;0],h,pcs,bcs,linjoints,Klib,wcomps,'r');
ari0 = Amat\Fv*Famp;

Copt = struct('Nmax', 100, 'angopt', 2e-1, 'DynDscale', 1);
ariwC = CONTINUE(@(ariw) WVHBRESFUN(ariw, Famp, h, pcs, bcs, nljoints, Klib, wcomps),  ...
...
aciC = zeros(size(acC));
for iw=1:size(acC,2)
    [Amat, ~, ~, Fv] = WVAMAT([acC(end,iw);0],h,pcs,bcs,linjoints,Klib,wcomps);
