...
%       dFNLdw  : (Nhc,1)
    ...
    ut = AFT(reshape(Uw(1:end-Nc), Nd, Nhc)', h, Nt, 'f2t');
    udt = AFT(D1*reshape(Uw(1:end-Nc), Nd, Nhc)', h, Nt, 'f2t');
    dudtw = cell2mat(arrayfun(@(a) AFT(dD1w(:,:,a)*reshape(Uw(1:end-Nc), Nd, Nhc)', h, Nt, 'f2t'), 1:Nc, 'UniformOutput', false));
    ...
    for di=1:Nd
        ...
        dFNLdw(di:Nd:end,:) = AFT(permute(sum(reshape(dfdud(:, di), Nd, Nt^Nc)'.*permute(dudtw, [1 3 2]),2), [1 3 2]), h, Nt, 't2f');
    end
