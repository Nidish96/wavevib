...
** Specification of a Library of Dispersion Relationships
<<sec:spec_klib>>
...
#+begin_Details "Dispersion of 1D Bars" :background-color "#c9c9c9" :title-color black
...
#+end_Details
#+begin_Details "Dispersion of 1D Euler-Bernoulli Beams" :background-color white :title-color black
...
#+NAME: eq:1debbmdispex
\begin{align}
k_{1,2}(\omega) &= \pm {\left(\frac{\rho A}{E_y I_y}\right)}^{1/4} = \pm k,\,\text{and}\\
k_{3,4}(\omega) &= \pm j {\left(\frac{\rho A}{E_y I_y}\right)}^{1/4} = \pm j k.
\end{align}
Unlike the case of the 1D Bar (eq. [[eq:1dbarex]]), the EB-Beam is described by four wave components out of which only two are real and the other two are imaginary. The fully real coefficients $k_{1,2}$ are interpreted as forward and backward traveling waves, just as in the case of the 1D Bar, but $k_{3,4}$ are non-traveling wave components and are referred to as _*evanescent waves*_ in the literature (see~[cite:@graffWaveMotionElastic2012], for instance). With the former denoted by $a^\pm$ and the latter by $b^\pm$, the wave based solution is expressed (using relative coordinates $x$) as
#+NAME: eq:1debbmwbsol
\begin{equation}
...
\end{equation}
where the model has $N$ wave components, each with a wavenumber $k_n$ and associated dispersion relationship $k_n(\omega)$. This is the format that =wavevib= expects the user to specify the different dispersion relationships by. 
...
#+begin_src octave
  ...
  wcomps_beam = [1  1;  % First component  -> exp(  k1 x )
		-1  1;  % Second component -> exp( -k1 x )
                 1j 1;  % Third component  -> exp( jk1 x )
		-1j 1]; % Fourth component -> exp(-jk1 x )

  wcomps_bar = [1j 2;   % First component  -> exp( jk2 x )
	       -1j 2];  % Second component -> exp(-jk2 x )
#+end_src
The =Klibs= variable is first constructed as an array of structs containing the dispersion relationships as function handles. Next, an $N\times 2$ array =wcomps_beam= is created to specify how to use the dispersion relationships for each wave coefficient in the beam, and the same is done for the bar also (through =wcomps_bar=). The first column of these arrays stores the coefficient in front of $k$ and the second column stores which element of the =Klibs= vector to choose for the current wave component. For instance, if a row is =[1j, 2]=, it says that the corresponding wave component's contribution to the solution is of the form $a e^{jkx} e^{-j\omega t}$.

Note that at this point the =wcomps_beam= and =wcomps_bar= arrays are merely arrays and no explicit relationship with =Klibs= has been specified. This will be done while specifying the different "Pieces" of the model, as covered in Sec. [[sec:spec_pcs]] below.
** Specification of the Pieces of a Model
<<sec:spec_pcs>>
...
*** Linear Joints
*** Nonlinear Joints
** Computation of Linear Analysis Quantities
*** Linear Jacobian and Force vector
*** Linear Modal Analysis
** Nonlinear Periodic Response Analysis
** Continuation of Nonlinear Periodic Responses
*** Stability of Nonlinear Solutions
** Post-Processing
** Advanced Interfaces
*** Minimalized Representation
*** Quasi-Periodic Response Analysis
* Examples
** Single Bar
<<sec:a_singbar>>
...
*** TODO Describe how to use WVEVALWCOFS for evaluating wave coefficients and plot deflection shape.
