...
%           'Gj', 'dGjdw', 'dGjdxi'
    ...
    for n=1:length(nlis)
        ...
        dLjdw{n} = zeros(joints(k).nld*Nh, Npts*Nwc*Nh);
        dLjdxi{n} = zeros(joints(k).nld*Nh, Npts*Nwc*Nh);

        Gj{n} = zeros(Npts*Nwc*Nh, joints(k).nld*Nh);
        dGjdw{n} = zeros(Npts*Nwc*Nh, joints(k).nld*Nh);
        dGjdxi{n} = zeros(Npts*Nwc*Nh, joints(k).nld*Nh);
    end
...
%                 dAmatdxi(si:se,qi:qe) = [diag(exp(K*dx).*dKdxi*dx) zeros(Nwc)];
                dAmatdw(si:se,qi:(qe-Nwc)) = diag(exp(K*dx).*dKdw*dx);
                dAmatdxi(si:se,qi:(qe-Nwc)) = diag(exp(K*dx).*dKdxi*dx);
        ...
        for n=1:length(bcs)
            ...
            if ~isempty(bcs(n).rih) && bcs(n).rih==h(ih)
                Fv(ktn+n) = Fv(ktn+n)+bcs(n).rhs(h(ih)*wxi(1), wxi(2));
                dFvdw(ktn+n) = dFvdw(ktn+n)+bcs(n).rhs(h(ih)*wxi(1), wxi(2))*h(ih);
                dFvdxi(ktn+n) = dFvdxi(ktn+n)+bcs(n).rhs(h(ih)*wxi(1), wxi(2));
            end
        end
        ...
        for n=1:nnl
            ...
            switch joints(k).type
                case 2
                    ...
                    Lj{n}((ih-1)*nld+(1:nld), inds) = joints(k).nldcofs(h(ih)*wxi(1), wxi(2));
                    dLjdw{n}((ih-1)*nld+(1:nld), inds) = joints(k).dnldcofsdw(h(ih)*wxi(1), wxi(2))*h(ih);
                    dLjdxi{n}((ih-1)*nld+(1:nld), inds) = joints(k).dnldcofsdxi(h(ih)*wxi(1), wxi(2));
    ...
    if length(varargin)>=1 && varargin{1}=='r'
        ...
        for n=1:nnl
            ...
            JEV(n).dLjdw = zeros(nld*Nhc, Npts*Nwc*Nhc);
            JEV(n).dLjdxi = zeros(nld*Nhc, Npts*Nwc*Nhc);
            
            JEV(n).Gj = zeros(Npts*Nwc*Nhc, nld*Nhc);
            JEV(n).dGjdw = zeros(Npts*Nwc*Nhc, nld*Nhc);
            JEV(n).dGjdxi = zeros(Npts*Nwc*Nhc, nld*Nhc);
